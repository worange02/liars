(function initGameEngine(global){
    class GameEngine {
        ...
        _bind(){
            ...
            n.on('REQUEST_STATE_SYNC', () => { 
                if(n.isHost && this.fullState) {
                    this._broadcastState();
                    this.net.sendToHost({ type: 'FULL_STATE_SYNC', fullState: JSON.parse(JSON.stringify(this.fullState)) });
                }
            });

            n.on('KEEPALIVE', () => { 
                if(n.isHost && this.fullState) {
                    const p = this._pByPeer(n.hostPeerId);
        ...
        // ─── Background mode for wake lock ───
        setBackgroundMode(isBackground) {
            if (isBackground) {
                this._isBackground = true;
                if (this.net && this.net.heartbeatTimer) {
                    clearInterval(this.net.heartbeatTimer);
                    this.net.heartbeatTimer = setInterval(() => {
                        const hb = { type: 'HEARTBEAT', t: Date.now() };
                        if (this.net.isHost) {
                            this.net.broadcast(hb);
                        } else if (this.net.hostConn && this.net.hostConn.open) {
                            this.net.hostConn.send(hb);
                        }
                    }, 30000);
                }
            } else {
                ...
                if (this.net && this.net.hostConn && this.net.hostConn.open) {
                    this.net.sendToHost({ type: 'REQUEST_STATE_SYNC' });
                }
            }
        }
        ...
        _saveFullState() {
            try {
                if (this.fullState) localStorage.setItem('lg_fullState', JSON.stringify(this.fullState));
            } catch(e) {}
        }
    }
