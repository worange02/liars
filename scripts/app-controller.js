(function initAppController(global){
    class App {
        constructor(){
            ...
            this._bindHome(); this._initRankGrid(); this._checkUrlParams();this._setupKeepAlive();
        }
...
_bindCallbacks(){
    ...
    this.net.onPeerConnect=(pid)=>{
        console.log('[App] Peer connected:', pid);
    };
    
    this.net.onPeerDisconnect=(pid)=>{
        console.log('[App] Peer disconnected:', pid);
        if(this.net.isHost && this.game) {
            this.game.handleDisconnect(pid);
        } else if(pid === this.net.hostPeerId) {
            this._hostLost();
            showToast('与房主断开连接，正在重连...', 3000);
        } else {
            showToast('有玩家断开连接', 2000);
        }
    };
}
        ...
        async _requestWakeLock() {
            ...
            document.addEventListener('visibilitychange', () => {
                if (!document.hidden && !this._wakeLock) {
                    this._requestWakeLock();
                }
            });
        }
        ...
        _onPageHide() {
            ...
            this._heartbeatTimer = setInterval(() => {
                if (this.net && this.net.hostConn && this.net.hostConn.open) {
                    this.net.sendToHost({ type: 'KEEPALIVE', ts: Date.now() });
                }
            }, 25000);
        }
